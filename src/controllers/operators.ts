...
export const loginOperator = async (req: Request, res: Response) => {
    try {
        ...
        await knex<Operator>("operator")
            .update({ open_service: true })
            .where({ email });

        const { id, name, email: Email, ticket_window, open_service } = operator;

        const token = jwt.sign({ id }, process.env.JWT_SECRET as string, { expiresIn: process.env.JWT_EXPIRES_IN });
        ...
        return res.status(200).json({
            usuario: {
                ...
                email: Email,
                ticket_window,
                open_queue: Boolean(open_service)
            },
            token,
        });
...
export const createOperators = async (req: Request, res: Response) => {
    try {
        ...
        const { id, name: Name, email: Email, ticket_window: Ticket_window, open_service, service_started } = operator;
...
export const updateOperators = async (req: Request, res: Response) => {
    try {
        ...
        const currentOperator = await knex<Operator>("operator").where({ id: userId }).first();
        ...
        const { name, email, password, ticket_window } = req.body;
        const hashedPassword = password ? await bcrypt.hash(password, 10) : currentOperator.password;
        ...
        const { id, name: Name, email: Email, ticket_window: Ticket_window, open_service, service_started } = newOperator;
        ...
        return res.status(200).json({
            usuario: {
                ...
                email: Email,
                ticket_window: Ticket_window,
                open_service: Boolean(open_service),
                service_started,
            },
        });
